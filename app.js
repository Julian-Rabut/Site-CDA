...
app.post("/client/login", async (req, res) => {
  try {
    res.locals.publicPage = true;
    const { email, mot_de_passe, next } = req.body;
    const nextUrl = sanitizeNextUrl(next || "/rdv");
...
app.post("/client/register", async (req, res) => {
  try {
    ...
    if (!nom || !email || !telephone || !mot_de_passe || !mot_de_passe_confirm) {
    ...
    const [result] = await pool.query(
      "INSERT INTO clients (nom, email, telephone, mot_de_passe) VALUES (?, ?, ?, ?)",
...
app.get("/client/mot-de-passe-oublie", (req, res) => {
  res.locals.publicPage = true;
...
app.post("/client/mot-de-passe-oublie", async (req, res) => {
  try {
    res.locals.publicPage = true;
...
app.get("/client/reset-password/:token", async (req, res) => {
  try {
    res.locals.publicPage = true;
...
app.post("/client/reset-password/:token", async (req, res) => {
  try {
    res.locals.publicPage = true;
    const token = req.params.token;
...
app.get("/client/compte", requireClientLogin, async (req, res) => {
  try {
    res.locals.publicPage = true;
...
app.post("/client/fiche", requireClientLogin, async (req, res) => {
  try {
    const clientId = req.session.client.id;
    const {
      nom,
...
app.post("/client/rdv/:id/annuler", requireClientLogin, async (req, res) => {
  let conn;
  try {
